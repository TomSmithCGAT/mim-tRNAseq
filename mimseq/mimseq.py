...
def mimseq(trnas, trnaout, name, species, out, cluster, cluster_id, cov_diff, posttrans, control_cond, threads, max_multi, snp_tolerance, \
	keep_temp, cca, double_cca, min_cov, mismatches, remap, remap_mismatches, misinc_thresh, mito_trnas, plastid_trnas, pretrnas, local_mod, search, stop_after_clustering, p_adj, sample_data):
...
# Main wrapper
	...
	= modsToSNPIndex(trnas, trnaout, mito_trnas, plastid_trnas, modifications, name, out, double_cca, threads, snp_tolerance, cluster, cluster_id, posttrans, pretrnas, local_mod, search)
...
def main():
	...
	options.add_argument('--no-cluster', required = False, dest = 'cluster', action = 'store_false',\
		help = 'Disable usearch/vsearch sequence clustering of tRNAs by isodecoder which drastically reduces the rate of multi-mapping reads. Default is enabled.')
	options.add_argument('--cluster-id', metavar = 'clustering identity threshold', dest = 'cluster_id', type = restrictedFloat, nargs = '?', default = 0.97,\
		required = False, help = 'Identity cutoff for usearch/vsearch clustering between 0 and 1. Default is 0.97.')
	...
	options.add_argument('--search', required=False, dest = 'search', choices = ['usearch', 'vsearch'], default='vsearch',
		help = "Which command to use to cluster tRNAs by isodecoders")
	options.add_argument('--stop-after-clustering', required=False, dest = 'stop_after_clustering', action='store_true',
				help = "Stop after isodecoder clustering. For debug purposes only")
	...
	parser.set_defaults(threads=1, out="./", max_multi = 3, mito = '', plastid = '', cov_diff = 0.5)
	...
	if len(sys.argv) <= 1:
		...
		if not args.species and not (args.trnas or args.trnaout):
			...
			mimseq(args.trnas, args.trnaout, args.name, args.species, args.out, args.cluster, args.cluster_id, args.cov_diff, \
				...
				args.misinc_thresh, args.mito, args.plastid, args.pretrnas, args.local_mod, args.search, args.stop_after_clustering, args.p_adj, args.sampledata)
