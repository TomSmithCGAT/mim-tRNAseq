...
def mimseq(trnas, trnaout, name, species, out, cluster, cluster_id, cov_diff, posttrans, control_cond, threads, max_multi, snp_tolerance, \
	keep_temp, cca, double_cca, min_cov, mismatches, remap, remap_mismatches, misinc_thresh, mito_trnas, pretrnas, local_mod, search, stop_after_clustering, p_adj, sample_data):
...
# Main wrapper
	...
	= modsToSNPIndex(trnas, trnaout, mito_trnas, modifications, name, out, double_cca, threads, snp_tolerance, cluster, cluster_id, posttrans, pretrnas, local_mod, search)
	...
	if len(sys.argv) <= 1:
		...
		if not args.species and not (args.trnas or args.trnaout):
			...
			mimseq(args.trnas, args.trnaout, args.name, args.species, args.out, args.cluster, args.cluster_id, args.cov_diff, \
				...
				args.misinc_thresh, args.mito, args.pretrnas, args.local_mod, args.search, args.stop_after_clustering, args.p_adj, args.sampledata)
